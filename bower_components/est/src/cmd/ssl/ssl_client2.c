/*
    ssl_client2.c -- SSL client with certificate authentication
...
int main(void)
{
     ...
     */
    ret = x509parse_crt(&cacert, (uchar *)xyssl_ca_crt,
                strlen(xyssl_ca_crt));
    if (ret != 0) {
    ...
    ret = x509parse_crt(&clicert, (uchar *)test_cli_crt,
                strlen(test_cli_crt));
    if (ret != 0) {
        printf(" failed\n  !  x509parse_crt returned %d\n\n", ret);
        goto exit;
    ...
    ret = x509parse_key(&rsa, (uchar *)test_cli_key,
